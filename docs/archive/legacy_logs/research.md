...
## 2026-05-08 Cline/LM Studio 88K general MCP uplift

- Updated `cline-gemma4-uplift-plan.md` from a newauto-only 24K/30K recovery plan into a general-purpose 88K Cline + LM Studio operating plan.
- Reloaded `google/gemma-4-e4b` in LM Studio with `--context-length 88000 --parallel 1 --gpu max`; `/api/v0/models` now reports `loaded_context_length=88000`.
- Registered a first general-purpose MCP set in both LM Studio and Cline:
  - `sequential-thinking` for planning and decomposition,
  - `memory` for short persistent preferences/procedures,
  - `filesystem` with root `C:/Users/petbl`,
  - `context7` for official library/framework documentation.
- Kept `newauto-stepwise` and `openclaw-operator` registered. The intended split is now: general MCPs for broad work, `newauto-stepwise` for the video pipeline, and `openclaw-operator`/fallback tools for local operator authority.
- Deliberately did not add `git-mcp-server` in the first pass because it emitted verbose stdout logs during startup and exposes a broad destructive git surface. Git remains available through `run_powershell`.
- Updated `scripts/newauto_stepwise_mcp.py` metadata `context_target` to `88000` and `.clinerules` to remove the stale 24K guidance.

## 2026-05-09 agentic self-improvement loop bootstrap

- Advanced the Cline/LM Studio authority plan from simple permission expansion toward a Codex-like iterative improvement loop.
- Expanded Cline MCP `autoApprove` for low-risk tools:
  - `sequential-thinking`, `memory`, read-only `filesystem`, `context7`,
  - `newauto-stepwise` workflow/diagnosis/search/Flow-control tools,
  - read/status/open tools in `openclaw-operator`.
- Kept higher-risk shell/write tools such as `run_powershell`, `write_text_file`, and filesystem write operations out of default Cline auto-approval.
- Added LM Studio loaded-context metadata to `scripts/newauto_stepwise_mcp.py` so `diagnose_runtime` can report:
  - `loaded_model`,
  - `loaded_context_length`,
  - `context_target_met`,
  - loaded model capabilities.
- Added `scripts/agent_eval_smoke.py` as a reusable agentic smoke/eval runner. It checks tool metadata, `diagnose_runtime`, Flow desktop readiness, `search_web`, and PowerShell command policy behavior, then writes JSON reports under `storage/agent_evals/`.
- Added `storage/agent_memory/lessons.jsonl` as the first repo-local self-improvement memory store. The intended schema is "symptom -> verified cause -> fix -> verification -> reusable lesson"; secrets and guesses must never be stored.
- Added model/profile prompts under `prompts/`:
  - `gemma4_agentic.md`,
  - `coding_reviewer.md`,
  - `workflow_operator.md`.
- Cleaned stale `.claude/settings.local.json` allowlist paths from `C:\Users\shs\audio\...` to the current `C:\Users\petbl\newauto\...` OmniVoice environment path.
- The first smoke run found a real metadata defect: `forensic_diagnose` existed but was missing from the visible tool metadata. The metadata/header list was corrected, and the follow-up full smoke passed with `loaded_context_length=88000`, `context_target_met=true`, Flow readiness true, `search_web` returning Ui.Vision/XModules official results, and destructive PowerShell policy returning `approval_required`.

## 2026-05-09 Cline/LM Studio/Gemma4 external upgrade research

- Created `cline-lmstudio-gemma4-upgrade-research-plan.md` after checking current Cline docs, LM Studio docs, Hugging Face Gemma4 materials, LM Studio/Cline GitHub issues, and recent MCP tool-use research.
- Key findings:
  - Cline official docs currently show MCP per-tool auto approval as `alwaysAllow`, while the local Cline config currently contains `autoApprove`; the next upgrade should verify which key the installed Cline build actually honors and normalize the config.
  - Cline supports MCP server timeout controls and separate config directories, so safe/research/workflow/yolo profiles can be split instead of using one permanent broad-permission setup.
  - LM Studio supports tool use and MCP, but GitHub issues show real parser/template/tool-call regressions, especially around streaming, `<think>` parsing, Gemma4/OpenClaw payloads, and repeated tool calls.
  - Gemma4 E4B is suitable as a fast 88K workflow operator, while 26B/31B should be tested as planner/reviewer profiles rather than replacing the stable stepwise operator blindly.
  - MCP research points toward semantic tool discovery/top-k tool selection and tool description quality, reinforcing the current narrow high-level `newauto-stepwise` hub design.
- The plan prioritizes:
  - config compatibility smoke,
  - LM Studio version/backend capture in eval reports,
  - Gemma4 parser/tool-call avoidance prompt rules,
  - model profiles,
  - GitHub CLI search,
  - tool catalog/top-k selection eval,
  - agent dashboard and loop detector,
  - MCP prompt-injection/security fixtures.

## 2026-05-10 Cline/LM Studio/Gemma4 upgrade plan v2.1 implementation

- Reviewed the v2 plan against the current codebase, `research.md`, `timeline.md`, and the actual workflow scripts, then implemented the first P0/P1 safety and self-improvement items.
- `scripts/agent_eval_smoke.py` now records richer agentic baseline data:
  - LM Studio loaded model/context/runtime/quantization and best-effort `lms` CLI version,
  - host Python executable/version,
  - Cline MCP config compatibility signals (`autoApprove`/`alwaysAllow`, disabled state, timeout, transport),
  - prior smoke regression diff,
  - repeated operator-call warnings from recent operator logs,
  - static MCP subprocess stdin guard,
  - port 9001 runtime mismatch warning,
  - structured lesson writing with secret redaction.
- Cleared the pre-existing hygiene blocker in `tests/test_feature_workflow.py` and confirmed `app/services/subtitle.py` still typechecks. Verification:
  - `python -m mypy app\services\subtitle.py`
  - `python -m pytest tests\test_feature_workflow.py -q`
  - `python -m py_compile tests\test_feature_workflow.py app\services\subtitle.py`
- Hardened MCP transport behavior by ensuring diagnostic/desktop subprocesses in `scripts/newauto_mcp.py` and `scripts/newauto_stepwise_mcp.py` do not inherit the MCP stdio pipe.
- Added practical self-improvement artifacts:
  - `prompts/model_profiles.md` for 8GB VRAM-aware model roles,
  - `storage/agent_memory/tool_catalog.json` plus `scripts/tool_catalog_eval.py`,
  - `storage/agent_memory/README.md` for the lesson schema,
  - `scripts/cline_settings_normalize.py` for Cline MCP config normalization preview.
- Tool catalog eval currently passes 10/10 fixtures (`hit_rate=1.0`), and `agent_eval_smoke.py --skip-web` exits cleanly.
- Current known remaining operational warning: port 9001 is still served by system Python 3.10 (`python.exe -m uvicorn app.main:app --host 127.0.0.1 --port 9001`) instead of the expected `omnivoice_env\Scripts\python.exe`. The new smoke records this as `port_9001_runtime_mismatch` rather than silently treating it as healthy.

## 2026-05-10 newauto agent API port moved to 9002

- Moved the MCP/newauto agent API default from port 9001 to port 9002 to avoid fighting the existing 9001 listener.
- Added `NEWAUTO_API_PORT` and `NEWAUTO_BASE_URL` as the runtime contract:
  - default `NEWAUTO_API_PORT=9002`,
  - default `NEWAUTO_BASE_URL=http://127.0.0.1:9002`.
- Updated:
  - `run-newauto-9001.cmd` to start uvicorn on `%NEWAUTO_API_PORT%` instead of hard-coded 9001,
  - `run-newauto-mcp.cmd` and `run-newauto-stepwise-mcp.cmd` to export the 9002 API env,
  - `scripts/newauto_mcp.py` to derive `BASE_URL` from `NEWAUTO_BASE_URL`/`NEWAUTO_API_PORT`,
  - `scripts/agent_eval_smoke.py` to check the configured API port instead of hard-coded 9001.
- Windows venv nuance: the TCP listener can be owned by a child system-Python process while its parent command is `omnivoice_env\Scripts\python.exe`. The smoke now treats the parent process path as valid evidence for the expected runtime.
- Verification:
  - `http://127.0.0.1:9002/health` returned `{"ok": true}`,
  - latest `agent_eval_smoke.py --skip-web` returned `ok=true`,
  - runtime check reported `api_port=9002`, `runtime_matches_expected=true`, and no warning.

## 2026-05-11 MakeLens Flow direct generation parity

- Reviewed the successful MakeLens Flow run artifacts under `C:\Users\petbl\MakeLens\pipeline_output\20260511_200155` and the implementation in `pipeline/experimental/flow_playwright_runner.py`.
- Key production pattern copied into newauto:
  - attach to the existing authenticated Flow Chrome session over CDP, defaulting to port `9225`,
  - reuse the proven MakeLens profile `C:\Users\petbl\music-auto\browser_profiles\automation_notebooklm` when it exists,
  - open a fresh Flow project for each prompt,
  - wait for a new visible result media node instead of guessing fixed sleeps,
  - save image bytes directly from blob/data/http/canvas sources instead of relying on the fragile download menu,
  - attach the saved local image to the newauto Flow manifest immediately.
- `scripts/flow_browser_automation.py generate` now defaults to this direct generate/save/attach path through `FLOW_DIRECT_GENERATE=1`.
- `scripts/newauto_mcp.py` now defaults `FLOW_AUTOMATION_BACKEND` to `playwright`; the stepwise workflow generates one missing Flow sentence per call using the direct path and advances to TTS when coverage is complete.
- Verification:
  - `python -m py_compile scripts\flow_browser_automation.py scripts\newauto_mcp.py scripts\newauto_stepwise_mcp.py scripts\lmstudio_openclaw_operator_mcp.py`
  - `python -m pytest tests\test_flow_uivision.py -q`

## 2026-05-11 Cline Flow backend override audit

- Checked the active Cline MCP settings at `C:\Users\petbl\AppData\Roaming\Code\User\globalStorage\saoudrizwan.claude-dev\settings\cline_mcp_settings.json`.
- No `FLOW_AUTOMATION_BACKEND=uivision` or `FLOW_MODE=uivision` env override was present in the Cline MCP server config.
- Added explicit launcher-level overrides anyway so inherited Windows/user/session env cannot force the old Ui.Vision route:
  - `run-newauto-stepwise-mcp.cmd`: `FLOW_AUTOMATION_BACKEND=playwright`, `FLOW_MODE=playwright`
  - `run-newauto-mcp.cmd`: `FLOW_AUTOMATION_BACKEND=playwright`, `FLOW_MODE=playwright`
- This makes Cline's `newauto-stepwise` MCP route deterministic: Flow uses the Playwright direct MakeLens-style path unless a file is intentionally edited again.
- Verification:
  - `python -m py_compile scripts\newauto_mcp.py scripts\newauto_stepwise_mcp.py scripts\flow_browser_automation.py`
  - `python -m pytest tests\test_flow_uivision.py -q`

## 2026-05-11 Flow direct failure root cause and live recovery

- Investigated project `d4599773c482` after Cline reported that Flow needed user login/permission confirmation.
- Actual failure was not authentication:
  - first blocker: the Flow settings popover (`Nano Banana Pro`, ratio/count panel) stayed open and intercepted pointer events over the prompt textbox,
  - second blocker after prompt submission: generated result URLs returned `401 Unauthorized` or browser-side `Failed to fetch` when read directly, even though the image was visible in the authenticated browser.
- Fixed `scripts/flow_browser_automation.py`:
  - removed `Nano Banana Pro`/model labels from image-tab selectors so the automation no longer opens the settings popover by mistake,
  - dismisses popovers before prompt entry,
  - focuses and types into the Slate/contenteditable prompt box without relying on a normal mouse click,
  - uses browser-context fetch for protected media URLs,
  - falls back to a Playwright element screenshot when protected media bytes cannot be fetched.
- Live verification on `d4599773c482`:
  - `continue-video-workflow` generated sentence 1 successfully,
  - attached `flow_sentence_001.png`,
  - coverage advanced to `1/6`,
  - generated file: `storage/projects/d4599773c482/flow_generated/flow_s001_20260511T212554.png`,
  - attached copies exist in `media/` and `flow_assets/`,
  - file dimensions: `225x400`, size `14532` bytes.
- Follow-up risk: screenshot fallback is lower resolution than MakeLens's normal `768x1376` output. It unblocks workflow state, but the direct path should later prefer opening the result detail/download path for higher quality when Flow blocks byte extraction.

## 2026-05-11 Flow direct 1K download path

- Added MakeLens-style Flow card/detail download handling to `scripts/flow_browser_automation.py`.
- The direct path now saves results in this order:
  - open the latest result card menu and click `Download` -> `1K`,
  - if needed, open the result detail view and click the download control,
  - fall back to protected media extraction,
  - finally fall back to the lower-resolution element screenshot.
- Downloaded Flow files keep their real extension, so JPEG/WebP/PNG downloads are attached with the matching filename instead of forcing `.png`.
- Live verification on project `d4599773c482`:
  - `continue-video-workflow` generated sentence 2,
  - saved `storage/projects/d4599773c482/flow_generated/flow_s002_20260511T213122.jpeg`,
  - attached `storage/projects/d4599773c482/media/flow_sentence_002.jpeg`,
  - dimensions are `768x1376`, size `553435` bytes,
  - coverage advanced to `2/6`, missing `[3, 4, 5, 6]`.
- Verification:
